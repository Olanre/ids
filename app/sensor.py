...
class Sensor(object):
    ...
    def __init__(self, name, sensorid, timeWindow, addressBaseline, portBaseline, degreeBaseline, threshold):
        """ Constructor
        """
        fname = "log/sensor-" + str(sensorid) + ".log"
        logging.basicConfig(
            filename=fname,
            level=logging.DEBUG, 
...
###################################################  Perform Alerting and Responses ###########################################################
    ...
    def _checkTriggerCrossed(self, entropyValues, entropyType):
        ...
        self.logger.debug("Upper and lower detection threshold for the {} are:  {} for upper  and :  {} for lower ".format(entropyType, upperTriggerThreshold, lowerTriggerThreshold))
    ...
    def processEntropyProfiler(self):
        self.logger.debug("Processing Anomaly Entropy Profile in Time Window {}".format(self.timeWindow))
        ...
        self.logger.debug(" Finished address Entropy in time Window: {}".format(self.timeWindow))

        entropies["port"] = self._processPortEntropy(firstPacketPortId, lastPacketId)
        self.logger.debug(" Finished port Entropy in time Window: {}".format(self.timeWindow))

        entropies["degree"] = self._processDegreeEntropy(firstPacketDegreeId, lastPacketId)
        self.logger.debug(" Finished degree Entropy in time Window: {}".format(self.timeWindow))
        ...
        for entropyType, entropyData in entropies.items():
            self.logger.debug("Checking if entropy data values {} for {} should contribute to an alert ".format(entropyData, entropyType))
            trigger = self._checkTriggerCrossed( entropyData, entropyType)
            if trigger:
                self.logger.debug("The set of entropy data within {} for {} will contribute to the trigger threshold ".format(entropyData,entropyType))
                triggerCount += 1
                if triggerCount >= alertThreshold:
                    self.logger.debug("Alert threshold {} reached with triggerCount of {}, rasing an alert on the sensor: {} in time window: {}".format(alertThreshold, triggerCount, self.sensorId, self.timeWindow))
                    #get the current timestamp 
                    theTime = self.getCurrentTimeStamp()
...
if __name__ == '__main__':
    try:
        sensorTest = Sensor("QRadar-Content", 5, 2, 0.4, 0.4, 0.6, 0.3)
